...
def scrape_top_250(url):
    ...
    '''
    # Empty list to append the URLs to
    movie_urls = []

    # Make a DOM from the TOP_250_URL page
    url = URL(TOP_250_URL)
    dom = DOM(url.download())

    # Get the URLs
    for a in dom.by_tag('tr')[1:251]:
        for b in a.by_tag('a')[:1]:
            movie_urls.append((abs(b.attributes.get('href',''), \
                base=url.redirect or url.string)).encode("utf-8"))
...
def scrape_movie_page(dom):
    ...
    # default values, in case no content is found
    title = '' 
    duration, rating, n_ratings = '-1', '-1', '-1' 
    genres, directors, writers, actors = [], [], [], []

    # get title
    for a in dom.by_tag('h1'):
        for b in a.by_tag('span')[:1]:
            title += plaintext(b.content)    
    ...
    # get votes (n_ratings), remove commas
    for a in dom.by_tag('div.star-box-details'):
        for b in a.by_tag('a')[:1]:
            n_ratings = (plaintext(b.content)[:-5]).replace(',', '')

    # get genres
    for a in dom.by_class('see-more inline canwrap')[1:]:
        for b in a.by_tag('a'):
            genres.append(plaintext(b.content))
    genres = seperator.join(genres)

    # get directors
    for a in dom.by_tag('div.txt-block')[:1]:
        for b in a.by_tag('span'):
            directors.append(plaintext(b.content))
    directors = seperator.join(directors)

    # get writers
    for a in dom.by_tag('div.txt-block')[1:2]:
        for b in a.by_tag('span'):
            writers.append(plaintext(b.content))
    writers = seperator.join(writers)

    # get actors
    for a in dom.by_tag('div.txt-block')[2:3]:
        for b in a.by_tag('span')[:-2]:
            actors.append(plaintext(b.content))
    actors = seperator.join(actors)
